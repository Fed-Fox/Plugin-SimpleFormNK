                    ...
                    if (Main.forms.getString(formid+".name").equals(((FormWindowSimple) e.getWindow()).getTitle())){
                        ...
                        for (Map<String, Object> buttname : FormsButtonsList) {
                            if (buttname.get("name").equals(((FormResponseSimple) e.getResponse()).getClickedButton().getText())){
                                if (buttname.containsKey("cmds")){
                                    if (buttname.get("cmds").toString().contains("@log")){
                                        String[] cmdsstr = String.valueOf(buttname.get("cmds")).split(" / ");
                                        for (String cmdstr :  cmdsstr){
                                            p.getServer().dispatchCommand(p.getServer().getConsoleSender(), cmdstr
                                                    .replace("@log ", "").replace("%p", p.getName()));
                                        }
                                    }else{
                                        String[] cmdsstr = String.valueOf(buttname.get("cmds")).split(" / ");
                                        for (String cmdstr : cmdsstr){
                                            p.getServer().dispatchCommand(p, cmdstr.replace("%p", p.getName()));
                                        }
                                    }
                                }else if (buttname.containsKey("cmd")){
